...
const ModalTexts = ({ activeTexts, state, setter }: ModalTextsProps) => {
    ...
    const handleDeleteText = (textToDelete: string) => {
        ...
        showConfirmation("¿DESEA ELIMINAR EL TEXTO?")
            .then((confirmed) => {
                if (confirmed) {
                    textToDelete.trim()
                    const updatedPhrases = activeTexts.filter(text => text !== textToDelete);
                    localStorage.setItem('texts', JSON.stringify(updatedPhrases));
                    setter(updatedPhrases);
                    setModalProps(null);
                } else {
                    setModalProps(null);
                }
    ...
    const handleDeleteTexts = () => {
        const showConfirmation = (question: string): Promise<boolean> => {
            return new Promise((resolve) => {
                setModalProps({ question, resolve });
            });
        };
        showConfirmation("¿DESEA ELIMINAR LOS TEXTOS?")
            .then((confirmed) => {
                if (confirmed) {
    ...
    return (
        <div className="absolute items-center justify-start gap-5 flex flex-col h-screen w-screen p-16 backdrop-blur-sm z-50 bg-[#000000af]">
            ...
            <div className="w-full flex justify-between">
                ...
                />
                <ButtonAnimation
                    state={() => handleDeleteTexts()}
                    disabled={activeTexts.length == 0}
                    text="BORRAR TODO"
            ...
            <div className="flex items-start justify-center w-full gap-2">
                ...
                <Scroll maxScrollValue={scrollMax} bgcolor={true} uniqueScroll={true} />
            </div>
        </div>
    )
}
