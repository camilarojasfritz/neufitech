        ...
        <div className="flex flex-row justify-around gap-12 w-full h-screen bg-zinc-900 p-12">
            <div className="h-full w-[10%] flex flex-col justify-between">
                ...
                <div className="flex flex-col gap-2">
                    <ButtonAnimation
                        disabled={isOff ? true : false} speakText="Si" text="SI" propClass="w-44 h-28" />
                    <ButtonAnimation
                        disabled={isOff ? true : false} speakText="No" text="NO" propClass="w-44 h-28" />
                </div>
            </div>
            ...
            <div className="flex flex-col justify-between gap-2 items-center w-[40%] bg-zinc-900">
                <div className="flex flex-row gap-2">
                    <ButtonAnimation
                        disabled={isOff ? true : false} speakText="Localizacion rapida" text="LOCALIZACION RAPIDA" propClass=" w-44 h-28" />
                    <ButtonAnimation
                        disabled={isOff ? true : false} state={() => functionState("girar")} speakText="Girar" text="GIRAR" propClass="w-44 h-28" />
                    <ButtonAnimation functionKeyboard={{ funct: 'changeIsOff', state: changeState }} speakText="Suspender" text="SUSPENDER" propClass="w-44 h-28" />
                </div>
                <div className="flex flex-col gap-2">
                    <div className="flex flex-row gap-2">
                        <div className="w-44 h-28 "></div>
                        <ButtonAnimation
                            disabled={isOff ? true : false} speakText="Arriba" propClass="w-44 h-28 flex justify-center items-center" imagen={{ src: flechaArriba, width: 60, height: 60 }} />
                        <div className="w-44 h-28 "></div>
                    </div>
                    <div className="flex flex-row gap-2">
                        <ButtonAnimation
                            disabled={isOff ? true : false} speakText="Izquierda" propClass="w-44 h-28 flex justify-center items-center" imagen={{ src: flechaIzquierda, width: 60, height: 60 }} />
                        <ButtonAnimation
                            disabled={isOff ? true : false} speakText="Abajo" propClass="w-44 h-28 flex justify-center items-center" imagen={{ src: flechaAbajo, width: 60, height: 60 }} />
                        <ButtonAnimation
                            disabled={isOff ? true : false} speakText="Derecha" propClass="w-44 h-28 flex justify-center items-center" imagen={{ src: flechaDerecha, width: 60, height: 60 }} />
                    </div>
                </div>
                <div className="flex flex-col gap-2">
                    <div className="flex flex-row gap-2">
                        <ButtonAnimation
                            disabled={isOff ? true : false} speakText="Dolor" text="DOLOR" color="bg-transparent" propClass="w-44 h-28" />
                        <ButtonAnimation
                            disabled={isOff ? true : false} speakText="Adormecido" text="ADORMECIDO" color="bg-transparent" propClass="w-44 h-28" />
                        <ButtonAnimation
                            disabled={isOff ? true : false} speakText="Hormigueo" text="HORMIGUEO" color="bg-transparent" propClass="w-44 h-28" />
                    </div>
                    <div className="flex flex-row gap-2">
                        <ButtonAnimation
                            disabled={isOff ? true : false} speakText="Picor" text="PICOR" color="bg-transparent" propClass="w-44 h-28" />
                        <ButtonAnimation
                            disabled={isOff ? true : false} speakText="Presion" text="PRESION" color="bg-transparent" propClass="w-44 h-28" />
                        <ButtonAnimation
                            disabled={isOff ? true : false} speakText="Molestia" text="MOLESTIA" color="bg-transparent" propClass="w-44 h-28" />
                    </div>
                    <div className="flex flex-row gap-2">
                        <ButtonAnimation
                            disabled={isOff ? true : false} speakText="Leve" text="LEVE" color="bg-amber-300" propClass="w-44 h-28" buttonBorder="border-amber-300" textColor="text-zinc-900" />
                        <ButtonAnimation
                            disabled={isOff ? true : false} speakText="Moderado" text="MODERADO" color="bg-orange-400" propClass="w-44 h-28" buttonBorder="border-orange-400" textColor="text-zinc-900" />
                        <ButtonAnimation
                            disabled={isOff ? true : false} speakText="Fuerte" text="FUERTE" color="bg-red-500" propClass="w-44 h-28" buttonBorder="border-red-500" textColor="text-zinc-900" />
                    </div>
                </div>
            </div>
        </div>
