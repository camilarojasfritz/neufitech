...
const VoiceRecognition: React.FC = () => {
  ...
  useEffect(() => {
    let interimArray = interimTranscript.trim().split(" ");
    if (interimArray[interimArray.length - 1].toLowerCase() === "stop") {
      stopRecognition();
    }
  }, [interimTranscript]);

  useEffect(() => {
    let finalArray = finalTranscript.trim().split(" ");
    if (finalArray[finalArray.length - 1].toLowerCase() === "stop") {
      finalArray.pop();
      setFinalTranscript(finalArray.join(" "));
    }
  }, [finalTranscript]);
  ...
  return (
    <div className="w-full h-full bg-white flex justify-between items-center rounded-lg text-black font-semibold">
      <div className="flex flex-col">
        <button
          className="w-[150px] max-h-[70px] border-green-400 bg-green-200 border-4 rounded-t-lg"
          onClick={startRecognition}
        >
          Comenzar grabación
        </button>
        <button
          className="w-[150px] max-h-[70px] border-red-400 bg-red-200 border-4 rounded-b-lg"
